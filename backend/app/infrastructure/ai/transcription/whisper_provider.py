...
class WhisperTranscriptionProvider(TranscriptionProvider):
    def __init__(self) -> None:
        ...
        self._sync_client = openai.OpenAI(api_key=settings.openai_api_key)
    ...
    async def transcribe(self, audio_path: str, duration_seconds: float = 3600.0) -> list[TranscriptChunkData]:
        ...
        try:
            response = await self._transcribe_async(normalized_path)
        except AssertionError as exc:
            if "Data should not be empty" not in str(exc):
                raise
            response = await asyncio.to_thread(self._transcribe_sync, normalized_path)

        return self._build_chunks(response)

    async def _transcribe_async(self, normalized_path: str):
    ...
    def _transcribe_sync(self, normalized_path: str):
        with open(normalized_path, "rb") as audio_file:
            return self._sync_client.audio.transcriptions.create(
                model=self._model,
                file=audio_file,
                response_format="verbose_json",
                timestamp_granularities=["segment"],
    ...
    def _build_chunks(self, response) -> list[TranscriptChunkData]:
